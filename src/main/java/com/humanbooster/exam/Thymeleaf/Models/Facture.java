...
public class Facture {
    ...
    public Double totalHt(){
        return lignes.stream().mapToDouble(ligne -> ligne.getPrixHt().doubleValue()*ligne.getQuantity().doubleValue()).sum();
    }

    public Double totalTtc(){
        return lignes.stream().mapToDouble(ligne -> ligne.getPrixHt().doubleValue()*ligne.getQuantity().doubleValue()*(1d+ligne.getTva().getTaux())).sum();
    }

    public LocalDate latestPaymentDatePossible(){
        return dateFacture.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
    }
}
